$(document).ready(function(){
    ...
    $("#sendEmailLink").click(function() {
        ...
        send2facode(sessionStorage.getItem("email_two_fa_url"), function() {
            $("#2fa_error").hide();
            $(".alert-success").show();
            $(".alert-success").html("Your code has been sent to your email id");
        });
    });
    ...
    $("#sendSMSLink").click(function() {
        send2facode(sessionStorage.getItem("mobile_two_fa_url"), function() {
            $(".alert-success").show();
            $("#2fa_error").hide();
            $(".alert-success").html("Your code has been sent to your mobile");
        });
    });
    ...
    $.ajax({
        ...
        success: function(msg){
            if(JSON.parse(JSON.stringify(msg)).is_2fa_enabled==1) {
                ...
                var mobile_two_fa_url = "http://localhost:8080/users/"+JSON.parse(JSON.stringify(msg)).id
                    +"/mobilenumbers/"+JSON.parse(JSON.stringify(msg)).mobile+"/2fa";

                var verify_2fa_url = "http://localhost:8080/users/"+JSON.parse(JSON.stringify(msg)).id
                    +"/codes/";
